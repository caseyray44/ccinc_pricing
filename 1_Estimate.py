...
st.set_page_config(page_title="CC Inc. Pricing Calculator", layout="wide")
...
with col5:
    siding = st.selectbox("Siding", ["Brick", "Vinyl", "Stucco"], key="siding")

col6, col7 = st.columns(2)
with col6:
    cleaning = st.selectbox("Cleaning", ["Soap/Scrub", "Soft Wash", "Pressure Wash"], key="cleaning")
with col7:
    ...
    if st.button("Save Estimate"):
        if not account_name:
            ...
            try:
                ...
                st.error(f"Failed to save estimate: {str(e)}")
