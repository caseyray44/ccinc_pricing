...
# JavaScript to close the multiselect dropdown after selection
st.markdown(
    """
    <script>
    document.addEventListener('DOMContentLoaded', function() {
        // Find the multiselect widget (assuming it's the only one; adjust selector if needed)
        const multiselect = document.querySelector('div[data-testid="stMultiSelect"]');
        if (multiselect) {
            const dropdown = multiselect.querySelector('div[role="combobox"]');
            const options = multiselect.querySelectorAll('li[role="option"]');

            options.forEach(option => {
                option.addEventListener('click', function() {
                    // Simulate a click outside to close the dropdown
                    setTimeout(() => {
                        const outside = document.createElement('div');
                        document.body.appendChild(outside);
                        outside.click();
                        document.body.removeChild(outside);
                    }, 100); // Small delay to ensure selection is registered
                });
            });
        }
    });
...
if st.button("Calculate"):
    ...
    if missing_details:
        ...
        total = max(total, 299.00)  # Apply truck minimum of $299
...
if "show_additional_services" in st.session_state and st.session_state.show_additional_services:
    ...
    if additional_services:
        ...
        if additional_results:
            ...
            # Recalculate total with truck minimum
            total = sum(st.session_state.results.values()) - st.session_state.results.get("total", 0)
            total = max(total, 299.00)  # Apply truck minimum
